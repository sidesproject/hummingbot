...
from hummingbot.connector.derivative.aster_perpetual.aster_perpetual_user_stream_data_source import (
...
class AsterPerpetualDerivative(PerpetualDerivativePyBase):
    ...
    def __init__(
            ...
            aster_main_address: str = None,
            aster_trading_private_key: str = None,
            aster_chain_id: int = 1666,
            trading_pairs: Optional[List[str]] = None,
            trading_required: bool = True,
            domain: str = CONSTANTS.DOMAIN,
    ):
        self.aster_main_address = aster_main_address
        self.aster_trading_private_key = aster_trading_private_key
        self.aster_chain_id = aster_chain_id
    ...
    def authenticator(self) -> AsterPerpetualAuth:
        return AsterPerpetualAuth(
            self.aster_main_address,
            self.aster_trading_private_key,
            self.aster_chain_id,
        )
    ...
    async def _place_cancel(self, order_id: str, tracked_order: InFlightOrder):
        ...
        if cancel_result.get("code") == CONSTANTS.UNKNOWN_ORDER_ERROR_CODE and \
                CONSTANTS.UNKNOWN_ORDER_MESSAGE in str(cancel_result.get("msg", "")):
            self.logger().debug(f"The order {order_id} does not exist on Aster Perpetuals. "
        ...
        while True:
            try:
                ...
                self.logger().network(
                    ...
                    exc_info=True,
                    app_warning_msg="Could not fetch user events from Aster. Check API key and network connection.",
                )
        ...
        if event_type == "ORDER_TRADE_UPDATE":
            ...
            for position in positions:
                ...
                try:
                    ...
                    continue
                existing_position = self._perpetual_trading.get_position(hb_trading_pair,
            ...
            for order_update, tracked_order in zip(results, tracked_orders):
                ...
                if isinstance(order_update, Exception) or "code" in order_update:
                    if not isinstance(order_update, Exception) and \
                            (order_update["code"] == CONSTANTS.ORDER_NOT_EXIST_ERROR_CODE
    ...
    async def _trading_pair_position_mode_set(self, mode: PositionMode, trading_pair: str) -> Tuple[bool, str]:
        ...
        if initial_mode != mode:
            ...
            if not (response.get("msg") == "success" and response.get("code") == 200):
                success = False
